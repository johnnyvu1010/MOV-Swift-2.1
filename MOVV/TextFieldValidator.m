...
-(void)drawRect:(CGRect)rect{
    ...
    CGContextSetRGBFillColor(ctx, 181, 181, 181, 1);
    CGContextSetShadowWithColor(ctx, CGSizeMake(0, 0), 7.0, [UIColor lightGrayColor].CGColor);
    ...
    CGRect imgframe=CGRectMake((showOnRect.origin.x+((showOnRect.size.width-30)/2)), ((showOnRect.size.height*0.7)+showOnRect.origin.y), 20, 10);
...
-(void)showErrorIconForMsg:(NSString *)msg{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"woof" object:nil];
    ...
    self.rightView.layer.sublayerTransform = CATransform3DMakeTranslation(-15, 0, 0);
    strMsg=[msg copy];
}
